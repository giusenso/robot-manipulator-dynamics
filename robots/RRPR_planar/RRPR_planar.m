...
%% ROBOT DATA
sigma = [0 0 1 0];           % 0 if revolute, 1 if prismatic
r{1} = [l(1)*cos(q(1)); l(1)*sin(q(1)); 0];
r{2} = [l(2)*cos(q(1)+q(2)); l(2)*sin(q(1)+q(2)); 0];
r{3} = [q(3)*cos(q(1)+q(2)); q(3)*sin(q(1)+q(2)); 0];
r{4} = [l(4)*cos(q(1)+q(2)+q(4)); l(4)*sin(q(1)+q(2)+q(4)); 0];
...
[vcm, rcm0, Tcm, M, c, christoff, S, Ucm, g] = ...
    subs_all(d(2), 0, vcm, rcm0, Tcm, M, c, christoff, S, Ucm, g);

[vcm, rcm0, Tcm, M, c, christoff, S, Ucm, g] = ...
    subs_all(l(2), 0, vcm, rcm0, Tcm, M, c, christoff, S, Ucm, g)
